...
pub async fn scraper_yt() {
    ...
    let video_selector =
        Selector::parse("div#contents.style-scope.ytd-rich-grid-renderer").unwrap();
    let video_name_selector = Selector::parse("a#video-title-link").unwrap();
    // let video_description_selector = Selector::parse(".post-description").unwrap();
    let video_link_selector = Selector::parse("#video-title-link").unwrap();
    ...
    for element in document.select(&video_selector) {
        ...
        let formated_link = format!("https://www.youtube.com/@ThrasherMag/videos{link}");
